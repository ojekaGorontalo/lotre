...
(function () {
  ...
  function getManualPrediction() {
    const data = historicalData.slice(0, 10); // 10 data terakhir
    if (data.length < 5) {
    ...
    // 3. Cari hot number (angka paling sering)
    let maxFreq = -1;
    let hotNumbers = [];
    ...
    // 4. Cari cold number (angka jarang muncul)
    ...
    // 5. Tentukan prediksi kategori (KECIL/BESAR)
    let prediction;
    if (kecil > besar) prediction = "KECIL";
    else if (besar > kecil) prediction = "BESAR";
    else {
      // Jika seri, lihat hasil terakhir dan ubah (pola rotasi)
      const lastResult = data[0].result; // data[0] adalah yang terbaru
      prediction = lastResult === "KECIL" ? "BESAR" : "KECIL";
    }

    // 6. Pilih angka prediksi berdasarkan hot number di kategori yang dipilih
    ...
    if (candidates.length > 0) {
      ...
      // Jika tidak ada hot di range, ambil cold di range
      candidates = coldNumbers.filter(n => range.includes(n));
      if (candidates.length > 0) {
        predictedNumber = candidates[0];
      } else {
        // Fallback: angka tengah range
  ...
  async function placeBet() {
    ...
    // === PREDIKSI MENGGUNAKAN METODE MANUAL ===
